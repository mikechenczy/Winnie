...
import com.mj.winnie.tools.TeleportationTools;
...
public class CommandTpDim implements Command<CommandSource> {
    ...
    public int run(CommandContext<CommandSource> context) throws CommandSyntaxException {
        ...
        if (player.getCommandSenderWorld().dimension().equals(ModDimensions.COMMUNISM_DIM)) {
            ServerWorld world = player.getServer().getLevel(World.OVERWORLD);
            TeleportationTools.teleport(player, world, new BlockPos(x, 200, z));
        } else {
            ServerWorld world = player.getServer().getLevel(ModDimensions.COMMUNISM_DIM);
            TeleportationTools.teleport(player, world, new BlockPos(x, 200, z));
        }
        return 0;
    }
